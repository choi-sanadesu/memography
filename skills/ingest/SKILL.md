...
- URL(기사·SNS 포스트·영상 링크 등)이나 파일(`.md`·`.pdf`·`.txt` 등)을 볼트에 편입할 때
...
| 파일 경로 | `01. inbox/article.pdf`, 절대경로 |
...
- **medium 축**: `type: source`인 페이지에 `medium`(`article | video | image | podcast | paper`) 필드를 기록한다. Step 3에서 URL host·확장자·MIME으로 자동 추론(상세: [references/classifier.md](references/classifier.md) "medium 추론"). SOT는 볼트 `CLAUDE.md §3.1`.
...
| # | 단계 | 처리 | B모드 확인 |
|---|---|---|---|
| 1 | 소스 읽기 | URL→fetch/Chrome, 파일→Read | — |
| 2 | 요약·핵심 논점 | LLM 추출 (1줄 요약 + 3~5 논점) | ✋ 방향 확인 |
| 3 | 도메인 분류 + medium 추론 | `_{domain}.md` 매칭 + URL host/확장자/MIME로 medium 판정 | ✋ 도메인 확정 (+ medium low/medium 시 함께 확인) |
| 4 | 원본 처리 | 파일→`{domain}/raw/` 자동 이동, URL→`source:`만 | 자동 |
| 5 | 소스 페이지 작성 | §4.2 템플릿 + `medium:` 필드, 제목 LLM 정제 | ✋ **신규 생성 확인** |
...
- **Sequential 실행** — 진짜 subagent 병렬은 추후. Chrome MCP 싱글 세션 + `index.md`·`log.md` 공유 쓰기 충돌 방지.
- **배치 확장자 필터 없음** — `--inbox-all`은 전체 파일 대상. 패턴·확장자 필터는 추후.
...
- **medium 필드는 v1.2.0에서 권장, 누락 시 `/lint` warning만**. v2.0.0에서 fail로 승격 예정 (단계적 마이그레이션).
