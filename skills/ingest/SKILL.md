...
- **media 축**: 모든 type 페이지에 선택적으로 `media`(`article | video | image | podcast | paper`) 필드를 기록할 수 있다. `media: null` 허용(분류 보류 시). Step 3에서 URL host·확장자·MIME으로 자동 추론(상세: [references/classifier.md](references/classifier.md) "media 추론"). SOT는 볼트 `CLAUDE.md §3.1`.
...
| 3 | 도메인 분류 + media 추론 | `_{domain}.md` 매칭 + URL host/확장자/MIME로 media 판정 | ✋ 도메인 확정 (+ media low/medium 시 함께 확인) |
| 4 | 원본 처리 | 파일→`{domain}/raw/` 자동 이동, URL→`source:`만 | 자동 |
| 5 | 소스 페이지 작성 | §4.2 템플릿 + `media:` 필드, 제목 LLM 정제 | ✋ **신규 생성 확인** |
