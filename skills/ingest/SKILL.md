...
| 5 | 소스 페이지 작성 | §4.2 템플릿, 제목 LLM 정제 | ✋ **신규 생성 확인** |
...
- **Sequential 실행** — 진짜 subagent 병렬은 v1.2. Chrome MCP 싱글 세션 + `index.md`·`log.md` 공유 쓰기 충돌 방지.
- **배치 확장자 필터 없음** — `--inbox-all`은 전체 파일 대상. 패턴·확장자 필터는 v1.2.
- 교차 도메인 소스는 1개 도메인만 선택. 부 도메인은 Step 6 링크로 연결.
- 영상 콘텐츠(YouTube·릴스)의 **영상 내용 자체 이해**는 out of scope. 설명·자막·댓글까지만.
- `--auto` 플래그(C모드 전환용)는 v2에서 도입 예정. v1.1은 B모드 고정.
