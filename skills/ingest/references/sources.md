...
대상 볼트의 `CLAUDE.md §8`(보안·준수 원칙)을 **반드시 준수**한다. 단 한 번의 경계 침범도 허용되지 않는다.
...
| 위치 | 처리 |
|---|---|
| `inbox/` 안의 파일 | 그대로 둠 (이미 inbox에 있음) |
| 볼트 밖 파일, 재사용 가치 (이미지·반복 참조) | "`raw/images/`로 복사할까요?" ✋ 확인 후 복사 |
| 볼트 밖 파일, 단발 | "inbox 옆에 복사할까요?" ✋ 확인 후 복사 |

볼트 §1·§7: `raw/images/`는 **재사용되는 이미지 영구 보관소**. 두 문서 이상에서 쓸 가능성이 있으면 여기. 단발이면 inbox 옆.

원본 이동 도구는 `Bash cp` 또는 동등 수단. **볼트 밖 원본은 이동 금지, 복사만**.

---

## 원제 추출 (Step 4 제목 정제 힌트)
...
**원제는 힌트에 불과.** 최종 슬러그는 LLM이 볼트 스타일로 정제:
...
4. 영상 자체 내용은 사용자 브리프에 의존 (v0.1 out of scope)
...
3. 이미 inbox에 있음 → 복사·이동 없음
4. Step 4에서 페이지 작성 (`inbox/{date}-{slug}.md`로 신규 생성, 원본 PDF는 옆에 그대로)
```
...
1. 볼트 밖 → 복사 확인
2. "재사용 가치 있음" 판단 → "raw/images/로 복사할까요?" ✋
3. 사용자 confirm → `raw/images/ref.jpg`로 복사 (원본 보존)
4. 복사된 경로에서 메타 추출:
   - sips -g pixelWidth -g pixelHeight -g format "raw/images/ref.jpg"
   - stat -f '%SB' -t '%Y-%m-%d' "raw/images/ref.jpg"
5. Read (비전) → 캡션 생성, 텍스트 인지 시 OCR 자동 전사
6. Step 4 → type: inbox + media: image 템플릿으로 frontmatter 채움
