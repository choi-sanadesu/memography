...
> **summary 길이 정책 주의**: 한국어 80자와 영문 80자는 정보량이 다름. 영문 source 위주 운영 시 `len()` 80~120 범위가 짧아 길이 부족 빈발 가능. v0.1은 단순 `len()` 기준 유지, 운영 데이터 누적 후 글자별 가중치 정책 재검토.
...
`inbox/`는 "유예 공간"(`CLAUDE.md §7 원칙`)이지만 누적되면 분류 부담 증가 (`§5.4` lint 항목).
