...
- **볼트 규칙은 대상 볼트의 `CLAUDE.md`에서 읽는다.** `domain` enum, `type` enum, `medium` enum(있으면), 필수 필드 목록, 제외 경로 등은 하드코딩하지 않는다. 규칙이 바뀌어도 스킬 수정 없이 동작해야 한다. 볼트 §3.1에 특정 enum이 없으면 해당 enum **값 검증은 skip**(warn도 fail도 아님), 다른 검증(필드 존재성·형식)은 계속 수행.
...
2. 볼트의 `CLAUDE.md`를 읽어 스키마 규칙 파악:
   - `domain` 유효값 (§3.1)
   - `type` 유효값 (§3.1)
   - `medium` 유효값 (§3.1, 정의돼 있으면. `type: source`에만 적용)
   - 필수 frontmatter 필드 (§3.1)
   - type별 조건부 필드 (§3.1 — 예: `type: source`는 `medium` 권장)
