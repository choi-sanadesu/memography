...
- `type` (페이지 역할): `entity | concept | case | retro | note | reference`
- `domain` (볼트 도메인): `enter | sana | archive | entity | project | meta`
- `media` (미디어 유형 — 모든 `type`에 선택 적용): `article | video | image | podcast | paper`

### media 적용 규칙 (v2.0.0)

- 어느 `type`의 페이지에든 `media` 필드를 둘 수 있다. type 제약 없음.
- `/ingest`는 URL host·파일 확장자·MIME 기반으로 media를 자동 추론한다 (상세: `skills/ingest/references/classifier.md` "media 추론" 섹션).
- `media: null`(또는 값 없는 키)은 유효 상태로 처리한다 — fail/warn 없음. 분류 보류 시 사용.
- enum 위반은 ❌ fail (볼트 §3.1에 media enum 정의 시).
...
| `/lint` Check 1   | enum 밖이면 fail (볼트 §3.1에 media enum 정의 시), null 허용    |
| `/promote`        | 선택 적용 (엔티티 페이지 필요 시 사용 가능)                     |
| `/retro`          | 선택 적용 (회고 페이지 필요 시 사용 가능)                       |
